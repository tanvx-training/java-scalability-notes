...
ptr = ary;             // (3)`,
    ...
    options: [
      "(1) không biên dịch được; (2) sập lúc chạy",
...
int **int_ptr;  // sizeof(int*) == 8`,
  ...
  return output;
    ...
    options: [
      "Không hợp lệ — đây là trả về con trỏ tới biến tự động trên stack",
      "Hợp lệ: `output` có cấp phát tĩnh, sống suốt vòng đời chương trình — nhưng chỉ có MỘT bộ đệm dùng chung cho mọi lời gọi",
      "Không hợp lệ vì `snprintf` đòi hỏi bộ đệm cấp phát trên heap",
      "Hợp lệ và mỗi lời gọi nhận một bản sao riêng của `output`",
    ],
    answer: 1,
...
char *next_ticket() {
  ...
  return result;
    ...
    options: [
      "`id` là `static` nên không tăng đúng giữa các lời gọi",
      "Trả về con trỏ tới mảng tự động — vùng nhớ stack hết hiệu lực ngay khi hàm trả về",
      "`sprintf` có thể tràn bộ đệm 20 byte khi in một `int`",
      "Thiếu `free(result)` nên hàm rò rỉ bộ nhớ mỗi lần gọi",
    ],
    answer: 1,
    explanation: "`result` là biến tự động: nó chỉ gắn với bộ nhớ stack trong thời gian sống của hàm, nên tiếp tục dùng vùng nhớ đó sau khi hàm trả về là một lỗi. Phương án C nghe rất hợp lý với ai quen cảnh giác `sprintf`, nhưng sách tính rõ: in một số nguyên \"không bao giờ vượt quá 11 ký tự kể cả byte NUL\", nên 20 byte là thừa. Phương án D sai vì `result` không hề được `malloc`. Cách sửa: dùng bộ nhớ heap, hoặc khai báo `output` là `static` như ví dụ ở §3.11. (§3.8.3, §15.1.3)",
  },
  {
    id: "spq009",
    field: "sysprog",
  ...
  while (*src) { dest = src; src++; dest++; }
    ...
    options: [
      "Sao chép đúng nội dung nhưng quên byte NUL kết thúc",
      "Nó chỉ dịch con trỏ `dest` sang trỏ vào chuỗi nguồn — không byte nào được sao chép; và byte NUL cũng không được chép",
      "Vòng lặp chạy quá một byte nên ghi tràn `dest`",
      "Không biên dịch được vì `src` phải khai báo `const char *`",
    ],
    answer: 1,
...
if (answer = 42) { printf("The answer is %d", answer); }`,
    ...
    options: [
      "Không in gì — điều kiện sai nên khối `if` bị bỏ qua",
      "\"The answer is 42\" — phép gán trả về chính giá trị được gán, và 42 khác không",
      "\"The answer is 3\"",
      "Lỗi biên dịch vì không được gán trong điều kiện `if`",
    ],
    answer: 1,
...
user_t *user = (user_t *) malloc(sizeof(user));`,
    ...
    options: [
      "100 byte — vừa đủ cho struct",
      "Kích thước của một con trỏ (8 byte trên hệ 64-bit), vì `user` là biến con trỏ chứ không phải kiểu",
      "Không biên dịch được vì `sizeof` cần tên kiểu",
      "104 byte — 100 byte dữ liệu cộng phần đệm căn chỉnh",
    ],
    answer: 1,
...
free(p);`,
    ...
    options: [
      ...
      "Hai lỗi (ghi qua con trỏ treo, rồi double free); phòng bằng cách gán `p = NULL` ngay sau `free`",
      "Không lỗi nào — `free` chỉ đánh dấu khối là trống nên vùng nhớ vẫn ghi được",
...
memcpy(buf + 2, buf, 8);`,
    ...
    options: [
      ...
      "Lỗi biên dịch vì `memcpy` nhận `void *` chứ không nhận `char *`",
...
int ok = 2 == sscanf(line, "%c %d", &type, &data);`,
    ...
    options: [
      ...
      "Phải truyền `data` chứ không phải `&data`: `sscanf` ghi đè chính biến con trỏ, khiến nó trỏ tới địa chỉ 10 và `free(data)` sau này sẽ hỏng",
      "`%c` phải đổi thành `%s` vì `type` sẽ nhận cả khoảng trắng",
      "Phải kiểm tra `sscanf` trả về 1 chứ không phải 2",
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "Vì không có cách nào kiểm soát độ dài được đọc, nên bộ đệm rất dễ bị tràn",
      "Vì nó không hoạt động khi standard input bị chuyển hướng từ file",
...
free(buffer);`,
    ...
    options: [
      ...
      "`getline` tự cấp phát và cấp phát lại một bộ đệm trên heap đủ lớn cho dòng đang đọc",
      "`getline` là system call nên nhanh hơn hàm thư viện `fgets`",
      "`getline` không bao giờ trả về -1 nên không cần kiểm tra lỗi",
    ],
    answer: 1,
...
if (parsed == 0) {
  // coi là lỗi phân tích
    ...
    options: [
      "Vì `strtol` trả về -1 khi lỗi chứ không phải 0",
      "Vì `strtol` trả về 0 cho cả chuỗi hợp lệ `\"0\"` lẫn chuỗi không hợp lệ; phải đặt `errno = 0` trước lời gọi rồi kiểm tra `errno` sau",
      "Vì phải dùng `strtoll` mới có mã lỗi trả về",
  ...
  },
  {
    ...
    options: [
      ...
      "Để tìm được khối liền TRƯỚC, nhờ đó `free` có thể gộp khối về cả hai phía",
      "Để căn chỉnh con trỏ trả về theo bội số của 16 byte",
      "Để lưu con trỏ `next` của explicit free list",
    ],
    answer: 1,
  ...
  // ===== sp-process — Tiến trình & Tín hiệu (spq023–spq036) =====
  {
    ...
    code: null,
    options: [
      "0 cho process con, PID của con cho process cha, -1 khi tạo process thất bại",
      "PID của cha cho process con, 0 cho process cha",
      "PID của process con cho cả hai bên",
      "1 cho process con, 0 cho process cha",
    ],
    answer: 0,
...
int main() {
  ...
  return 0;
    ...
    options: [
      ...
      "\"Answer: 42\" hai lần — bộ đệm stdout chưa được flush đã bị nhân bản cùng toàn bộ bộ nhớ process",
      "Không in gì cả vì chuỗi thiếu ký tự xuống dòng",
  ...
  if (child == 0) {
    ...
    options: [
      ...
      "1024 process — một fork bomb; sửa bằng cách thêm `exit` ngay sau `exec` để process con không quay lại vòng lặp fork",
      "Đúng 10 process; `exec` thất bại nên các con chỉ đơn giản không làm gì",
      "20 process, vì mỗi vòng lặp `fork` được gọi hai lần",
    ],
    answer: 1,
  ...
  },
  {
    ...
    code: null,
    options: [
      "Zombie; nếu cha chết, con được `init` (PID 1) nhận nuôi, và `init` tự động wait nên zombie được dọn sạch",
      "Orphan (mồ côi); nó bị kernel kill ngay lập tức",
      "Zombie; nó tiếp tục được cấp thời gian CPU cho tới khi bị `kill -9`",
      "Orphan; PPID của nó trở thành 0 và nó chạy mãi",
    ],
    answer: 0,
  ...
  },
  {
    ...
    options: [
      ...
      "Bản sao của signal disposition (handler `SIGINT`) và của signal mask (chặn `SIGQUIT`), nhưng KHÔNG thừa hưởng signal đang pending",
      "Signal pending và mask, nhưng handler bị đặt lại về mặc định",
      "Không thừa hưởng gì cả — process con khởi đầu với trạng thái signal sạch trơn",
    ],
    answer: 1,
...
while ((nread = getline(&buffer, &buffer_cap, file)) != -1) {
  ...
  wait(NULL);
    ...
    options: [
      ...
      "Hành vi không xác định: `FILE*` chưa được \"chuẩn bị\" trước khi fork; phải `fflush` (hoặc đóng) nó trước",
      "Rò rỉ bộ nhớ vì `buffer` không bao giờ được `free` trong process con",
      "Đây là fork bomb vì `fork` nằm trong vòng lặp",
    ],
    answer: 1,
  ...
  return 0;
    ...
    options: [
      ...
      "Cả \"Captain's log\" lẫn danh sách `/usr/include` đều đi vào `log.txt`, vì `open` lấy file descriptor trống thấp nhất — chính là 1 — nên stdout đã bị chuyển hướng",
      "`execl` thất bại và `perror` chạy, vì thư mục làm việc đã bị đổi trước lời gọi",
      "`log.txt` nhận danh sách thư mục gốc của chương trình chứ không phải `/usr/include`",
    ],
    answer: 1,
  ...
  },
  {
    ...
    code: null,
    options: [
      "Vì `exec` không chạy được nếu chương trình chưa `fork` trước",
      "Vì `exec` thay thế ảnh process; `fork` trước để process cha còn sống, giám sát process con và làm tiếp việc khác",
      "Vì `fork` nhanh hơn `exec` nhờ cơ chế copy-on-write",
      "Vì mọi system call đều bắt buộc phải đi kèm một lời gọi `wait`",
    ],
    answer: 1,
  ...
  system(to_exec);
    ...
    options: [
      ...
      "Vì `system` sinh hẳn một shell rồi đưa chuỗi cho shell đó — tốn kém hơn exec trực tiếp và mở đường cho leo thang đặc quyền khi chuỗi chứa dữ liệu người dùng",
...
if (pid != -1 && WIFEXITED(status)) {
  ...
  printf("Process %d returned %d", pid, exit_status);
    ...
    options: [
      "Vì `WEXITSTATUS` trả về -1 khi process bị kết thúc bởi signal",
      "Vì các macro không tự kiểm tra tiền điều kiện: giá trị lấy ra vô nghĩa nếu process không kết thúc theo cách mà macro giả định",
      "Vì `waitpid` có thể trả về trước khi process con thực sự thay đổi trạng thái",
      "Vì exit status chỉ có 8 bit nên phải dịch bit thủ công trước khi dùng",
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "Vì `printf` dùng `malloc`: nếu signal đến đúng lúc chương trình đang ở giữa mã của `malloc`, các cấu trúc bộ nhớ đang ở trạng thái không nhất quán → hành vi không xác định",
      "Vì stdout bị chặn trong khi handler đang chạy nên `printf` sẽ block vĩnh viễn",
      "Vì signal handler không được phép gọi bất kỳ system call nào",
    ],
    answer: 1,
    explanation: "Vấn đề là tính **re-entrant**: hàm phải chịu được việc bị đóng băng giữa chừng rồi được gọi lại từ đầu. `printf` gọi `malloc`, và nếu handler chen vào lúc `malloc` đang sửa dở cấu trúc dữ liệu nội bộ, lời gọi thứ hai sẽ thấy trạng thái nửa vời. Phương án D là lời khái quát quá tay và cũng là hiểu lầm hay gặp: `write` là system call và **được** dùng trong handler — chính sách dùng `write(1, \"signaled!\", 9)` trong ví dụ handler. Cách an toàn là chỉ đặt một cờ `volatile sig_atomic_t` rồi để chương trình chính xử lý. (§13.3)",
  },
  {
    id: "spq035",
    field: "sysprog",
...
int main() {
  ...
  /* clean up code here */
    ...
    options: [
      ...
      "Vì trình biên dịch có thể tối ưu bỏ phép kiểm tra và giá trị có thể bị cache trong thanh ghi CPU; khai báo `volatile sig_atomic_t pleaseStop`",
  ...
  },
  {
    ...
    domain: "sp-process",
    difficulty: 1,
    question: "Signal nào KHÔNG thể bị bắt bởi một signal handler?",
    code: null,
    options: [
      "`SIGINT`",
      "`SIGTERM`",
    ...
    answer: 2,
    explanation: "Bảng signal POSIX trong sách ghi rõ `SIGKILL` là \"Terminate Process (Cannot be caught)\" và `SIGSTOP` là \"Stop Process (Cannot be caught)\" — đây là cách hệ thống bảo đảm luôn dừng hay giết được một process bất trị. `SIGINT` và `SIGTERM` đều bắt được (cột của chúng ghi \"Can be caught\"). `SIGSEGV` cũng bắt được, và đó là phương án dễ chọn nhầm nhất vì ta quen thấy nó làm sập chương trình — nhưng sập chỉ là **hành động mặc định**, không phải điều bắt buộc. Cũng vì thế sách khuyên gửi 15 rồi mới tính tới `kill -9`: `SIGKILL` không cho process kịp dọn dẹp. (§13.1, §15.10)",
  },
  ...
  },
  {
    ...
    code: null,
    options: [
      "Dùng chung stack; mỗi thread có heap riêng",
      "Dùng chung không gian bộ nhớ ảo — heap, biến toàn cục, mã chương trình; mỗi thread có stack riêng",
      "Dùng chung mọi thứ, kể cả stack",
  ...
  *p_int = x;
  return NULL;
    ...
    options: [
      "Luôn luôn 4",
      "4 nếu hai thread không chồng lấn, nhưng có thể là 2 khi các lệnh đan xen — đây là race condition",
      "Luôn luôn 2",
      "Lỗi biên dịch vì hai thread không được nhận cùng một con trỏ",
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "Có — chỉ một lệnh assembly thì không thể bị đan xen",
      "Không — bản thân phần cứng vẫn có thể gặp race vì ta chưa bảo nó kiểm tra; cần thêm tiền tố `lock`",
      "Có, nhưng chỉ trên máy một lõi",
      "Không, vì `-O2` không bảo đảm lúc nào cũng sinh ra lệnh này",
    ],
    answer: 1,
...
int main() {
  ...
  pthread_exit(NULL);
    ...
    options: [
      ...
      "Vì mọi thread nhận cùng một con trỏ `&i`, mà `i` vẫn đang bị vòng lặp thay đổi khi các thread khởi động",
      "Vì các thread chạy không theo thứ tự được tạo",
  ...
  },
  {
    ...
    options: [
      ...
      "`pthread_exit` chỉ kết thúc thread gọi nó, process sống tiếp tới khi mọi thread khác xong; `exit` kết thúc cả process và dừng mọi thread bên trong",
  ...
  },
  {
    ...
    code: null,
    options: [
      "Không sao — tài nguyên của thread luôn được giải phóng ngay khi hàm thread trả về",
      "Các thread đã kết thúc vẫn tiếp tục tiêu tốn tài nguyên; tạo đủ nhiều thì `pthread_create` sẽ thất bại",
      "Chương trình sẽ deadlock ở lần tạo thread thứ hai",
...
pthread_mutex_unlock(&m2);`,
    ...
    options: [
      "Có — mỗi thread đều khoá một mutex trước khi ghi",
      "Không — mutex khoá **mã** chứ không khoá **biến**; hai thread khoá hai mutex khác nhau nên vẫn vào vùng găng cùng lúc",
      "Có, miễn là `m1` và `m2` được khởi tạo trước khi tạo thread",
      "Không, vì `a` cần được khai báo `volatile` thì mutex mới có tác dụng",
    ],
    answer: 1,
  ...
  },
  {
    ...
    domain: "sp-concurrency",
    difficulty: 2,
    question: "Nỗ lực làm stack thread-safe dưới đây chứa hai lỗi. Chúng là gì?",
    code: {
      lang: "c",
...
int is_empty() {
  ...
  pthread_mutex_unlock(&m1);
    ...
    options: [
      "`push` và `pop` dùng hai mutex khác nhau nên vẫn chạy song song; và `is_empty` `return` trước khi `unlock` nên `m1` không bao giờ được mở khoá",
      "Chỉ có một lỗi: thiếu kiểm tra tràn và thiếu stack",
      "`is_empty` không cần khoá vì nó chỉ đọc; lỗi duy nhất nằm ở `pop`",
      "`push` và `pop` cần chung mutex, còn `is_empty` đã hoàn toàn đúng",
    ],
    answer: 0,
...
lower my flag`,
  ...
  },
  {
    ...
    code: null,
    options: [
      "Khoá mutex, ngủ, rồi trả về với mutex vẫn đang khoá",
      "Mở khoá mutex, ngủ tới khi được signal/broadcast, rồi khoá LẠI mutex trước khi trả về",
      "Ngủ trước, sau đó khoá mutex, rồi mở khoá nó",
  ...
  },
  {
    ...
    domain: "sp-concurrency",
    difficulty: 2,
    question: "Nỗ lực dùng condition variable cho `push` dưới đây mắc ba lỗi. Chúng là gì?",
    code: {
      lang: "c",
  ...
  pthread_mutex_unlock(&s->m);
    ...
    options: [
      "Điều kiện phải so với dung lượng tối đa chứ không phải 0; phải dùng `while` chứ không phải `if` vì có spurious wakeup; và không bao giờ signal cho ai nên thread có thể chờ vô hạn",
      "Chỉ thiếu một lời gọi `pthread_cond_signal` ở cuối hàm",
      "Mutex phải được mở khoá trước khi gọi `cond_wait`",
      "Phải thay `cond_wait` bằng `cond_broadcast` và bỏ hẳn mutex",
    ],
    answer: 0,
  ...
  },
  {
    ...
    code: null,
    options: [
      "Một lỗi của thư viện pthread, sẽ hết nếu nâng cấp libc",
      "Thread đang chờ tỉnh dậy dù chưa có lời gọi signal tương ứng; nó tồn tại vì hiệu năng — kernel không phát hiện được signal bị mất nhưng phát hiện được khi điều đó **có khả năng** xảy ra, nên đánh thức thread để mã kiểm tra lại điều kiện",
      "Việc bộ lập lịch đánh thức thread để nó nhường CPU cho thread ưu tiên cao hơn",
  ...
  },
  {
    ...
    code: null,
    options: [
      "Vì `cond_signal` chỉ đánh thức MỘT thread; nếu nhiều người đọc đang chờ người ghi xong thì chỉ một người đọc được dậy",
  ...
  },
  {
    ...
    code: null,
    options: [
      "Deadlock; sửa bằng cách đảo thứ tự `lock` và `cond_wait`",
      "Người ghi bị bỏ đói (starvation) vì biến đếm `reading` không bao giờ về 0; sửa bằng cách thêm biến đếm `writers` để người đọc đến SAU khi người ghi đăng ký phải chờ trong \"chuồng chờ\"",
      "Race condition trên biến `reading` vì nó được tăng ngoài vùng găng",
      "Người đọc bị bỏ đói vì mỗi lần người ghi xong nó lại giành khoá ngay",
    ],
    answer: 1,
  ...
  },
  {
    ...
    domain: "sp-concurrency",
    difficulty: 2,
    question: "Bản phác thảo barrier dưới đây có hai khiếm khuyết. Chúng là gì?",
    code: {
      lang: "c",
      text: `// Global:
int remain = N;
...
if (remain == 0) {
  ...
  while (remain != 0) { /* spin spin spin */ }
    ...
    options: [
      "`remain` phải được khởi tạo bằng 0 rồi tăng dần lên N",
      "Hai thread có thể cùng giảm `remain` (phép giảm không nguyên tử), và vòng chờ là một vòng lặp bận đốt CPU vô ích",
      "Chỉ cần đổi `while` thành `if` là đủ",
      "Thiếu `pthread_join` nên thread chính thoát trước khi các thread khác tới barrier",
    ],
    answer: 1,
  ...
  pthread_mutex_unlock(&b->m);
    ...
    options: [
      ...
      "Vì một \"thread tham vọng\" có thể thoát khỏi vòng chờ, làm xong phần tính của mình, đặt lại `remain` rồi quay về ngủ trước khi các thread khác kịp tỉnh — chúng không bao giờ tỉnh nữa",
      "Vì mutex bị huỷ ngay sau lần broadcast đầu tiên",
...
lower my flag`,
    ...
    options: [
      "Vì thuật toán chỉ đúng cho hai thread, không mở rộng được",
      "Vì các lệnh được bộ xử lý thực thi không theo thứ tự (out-of-order), phá vỡ giả định về trình tự đọc/ghi mà chứng minh dựa vào",
      "Vì biến `turn` phải là kiểu nguyên tử thì thuật toán mới đúng",
      "Vì nó vi phạm Bounded Wait khi có nhiều hơn hai thread cùng tranh chấp",
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "Với semaphore, một thread có thể `sem_post` để \"mở khoá\" cái mà thread khác đang giữ; mutex chỉ cho phép chính thread đã khoá được mở khoá",
      "Mutex đếm được nhiều hơn 1 còn semaphore thì không",
      "Semaphore chỉ dùng được giữa các process, không dùng được giữa các thread",
    ],
    answer: 1,
...
void *singsong(void *param) {
  ...
  return NULL;
    ...
    options: [
      ...
      "`sem_post`",
      "`printf`",
      "Cả ba đều dùng được vì chúng đều thuộc thư viện chuẩn",
    ],
    answer: 1,
...
void *dequeue() {
  ...
  return result;
    ...
    options: [
      "Hoạt động đúng nhưng chậm vì giữ mutex quá lâu",
      "Mỗi hàm wait rồi post trên CÙNG một semaphore nên giá trị không đổi: `s1` không bao giờ về 0 → `enqueue` không chặn khi buffer đầy, gây overflow; còn `s2` khởi tạo bằng 0 nên `dequeue` chặn vĩnh viễn",
      "Vấn đề duy nhất là thiếu loại trừ lẫn nhau, dẫn tới race condition trên `in` và `out`",
      "Underflow xảy ra vì `out` có thể vượt qua `in`",
    ],
    answer: 1,
  ...
  },
  {
    ...
    domain: "sp-concurrency",
    difficulty: 2,
    question: "Cài đặt ring buffer thứ hai dưới đây mắc ba vấn đề. Chúng là gì?",
    code: {
      lang: "c",
      text: `void *b[16];
int in = 0, out = 0;
...
void *dequeue() {
  ...
  return result;
    ...
    options: [
      "`enqueue` chặn ngay dù buffer đang rỗng (`s2` bằng 0); `dequeue` KHÔNG chặn nên đọc rác — underflow (`s1` bằng 16); và `lock` chưa bao giờ được khởi tạo nên loại trừ lẫn nhau không được bảo đảm",
      "Chỉ cần đổi tên hai semaphore cho nhau là đoạn mã hoàn toàn đúng",
      "Buffer sẽ overflow sau đúng 16 phần tử vì không ai kiểm tra `in` so với `out`",
      "`in` và `out` sẽ tràn số nguyên sau hơn bốn tỷ thao tác nên chỉ số quay về 0 sai chỗ",
    ],
    answer: 0,
...
void enqueue(void *value) {
  b[(in++) % N] = value;
    ...
    options: [
      "Phép `%` chậm; nên dùng `& (N-1)` để chương trình nhanh hơn",
      "Sau hơn bốn tỷ thao tác enqueue, `in` tràn và quay về 0 nên có thể ghi nhầm ô; dạng đúng dùng mặt nạ bit `b[(in++) & (N-1)]` với điều kiện N là luỹ thừa của hai",
      "`in++` bên trong chỉ số mảng là hành vi không xác định trong C",
      "Phải dùng tiền tố `++in` thay vì hậu tố `in++` thì chỉ số mới đúng",
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "Không — sau `fork` mỗi bên có một mutex độc lập; muốn dùng chung phải đặt mutex trong vùng nhớ chia sẻ (`mmap` với `MAP_SHARED`) và khởi tạo với thuộc tính `PTHREAD_PROCESS_SHARED`",
