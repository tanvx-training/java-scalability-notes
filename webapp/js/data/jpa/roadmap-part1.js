...
**Đọc.** [Ánh xạ entity với identity](#/docs/jpa-05) đọc lướt phần mở đầu để nắm lộ trình mục 5.2. [Hiểu về identity và equality trong Java](#/docs/jpa-05) đọc chậm, ghi lại đúng ba khái niệm: object identity (\`==\`), object equality (\`equals()\`), database identity (so sánh table và giá trị primary key) — bộ ba này đã gặp ở tuần 1 dưới góc paradigm mismatch (chương 1), ở đây là góc ánh xạ (chương 5). [Entity class và ánh xạ đầu tiên](#/docs/jpa-05) đọc kỹ và gõ lại listing \`Item\` với \`@Entity\`/\`@Id\`/\`@GeneratedValue\`, chú ý đoạn giải thích vì sao Hibernate hay Spring Data JPA dùng Hibernate truy cập field thay vì getter/setter khi \`@Id\` nằm trên field.
