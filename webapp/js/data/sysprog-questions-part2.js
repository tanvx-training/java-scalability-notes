...
pthread_mutex_lock(m1); // blocks`,
  ...
  },
  {
    ...
    answer: 0,
    explanation: "Mỗi \"khoản vay\" chỉ được chấp thuận khi hệ thống vẫn ở trạng thái an toàn — tức vẫn còn đủ tiền để đáp ứng ít nhất một người nữa — nên ta luôn có thể đi thêm một bước và hệ thống không bao giờ deadlock. Nhưng sách nói rõ ngay sau đó: \"không có gì đảm bảo hệ thống sẽ không livelock\" — nếu process mà ta trông chờ lại không bao giờ đưa ra yêu cầu thì chẳng có việc gì được làm. Phương án C bỏ qua ba nhược điểm sách liệt kê: phải biết trước nhu cầu của mỗi process, chạy chậm khi có hàng triệu tài nguyên, và không theo dõi được tài nguyên đến rồi đi. (§8.3, §17.4)",
  },
  {
    id: "spq070",
    field: "sysprog",
...
if (mychild > 0) {
  ...
  },
  {
    ...
    domain: "sp-memory-ipc",
    difficulty: 1,
    question: "Chương trình sau in ra đủ các ký tự nhưng không bao giờ kết thúc. Vì sao?",
    code: {
      lang: "c",
      text: `pid_t p = fork();
if (p > 0) {
  ...
  while ((bytesread = read(fd[0], &buf, 1)) > 0) {
    ...
    options: [
      ...
      "`putchar` đệm đầu ra nên các ký tự không bao giờ tới màn hình",
      "`wait(NULL)` ở process cha chặn trước khi kịp ghi vào pipe",
    ],
    answer: 0,
    explanation: "Sách nêu quy tắc: \"khi một process cố đọc từ một pipe mà vẫn còn bên ghi, process sẽ block. Nếu pipe không còn bên ghi nào, `read` trả về 0\". Ở đây cả process cha lẫn process con đều còn mở `fd[1]`, nên vòng lặp không bao giờ nhận được 0. Phương án B đảo ngược giá trị trả về — điều kiện `> 0` sẽ thoát ngay nếu `read` thật sự trả về -1, nên nó không giải thích được hiện tượng treo. Hai cách sửa của sách: sau khi fork, mỗi bên đóng đầu pipe mình không dùng; hoặc thoát vòng lặp khi gặp một dấu hiệu kết thúc thông điệp. (§9.3.1)",
  },
  {
    id: "spq084",
    field: "sysprog",
...
$ chmod 700 /tmp/mystuff`,
  ...
  *((&p)+2) = breakout;
  ...
  },
  {
    ...
    answer: 0,
    explanation: "Cơ chế SSL Heartbeat rất đơn giản: một server gửi một chuỗi có độ dài nhất định và server kia phải gửi lại chuỗi có độ dài đó. Sách mô tả lỗ hổng là \"không hề có kiểm tra giới hạn trên buffer\": kẻ tấn công khai độ dài lớn hơn những gì thực sự gửi — ví dụ gửi \"cat\" nhưng yêu cầu 500 byte — và lấy được những thông tin quan trọng như mật khẩu từ server. Vì dữ liệu chỉ bị **đọc** ra chứ không bị sửa, yếu tố bị phá vỡ là tính bảo mật của thông tin. Phương án B là nhầm lẫn hay gặp nhất vì \"buffer overflow\" thường gợi tới ghi tràn. Bài học của sách: kiểm tra buffer, và biết sự khác nhau giữa một buffer và một string. (§18.2, §14.1.1, §14.5)",
  },
  {
    id: "spq107",
    field: "sysprog",
