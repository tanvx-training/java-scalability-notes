...
**Đọc.** [Consumer Groups](#/docs/kafka-12) cùng khung "Consumer group dựa trên ZooKeeper (ZooKeeper-based consumer groups)" — biết vì sao đừng đụng \`--zookeeper\`. [Liệt kê và mô tả group (List and Describe Groups)](#/docs/kafka-12) chạy cả \`--list\` lẫn \`--describe --group\`, đọc Bảng 12-1 ba cột \`CURRENT-OFFSET\`, \`LOG-END-OFFSET\` và \`LAG\`. [Xóa group (Delete Group)](#/docs/kafka-12) ngắn: group phải rỗng mới xóa được. [Quản lý offset (Offset Management)](#/docs/kafka-12) nặng nhất, gồm [Xuất offset (Export offsets)](#/docs/kafka-12) — nhớ lệnh này thiếu \`--dry-run\` sẽ reset offset thật — và [Nhập offset (Import offsets)](#/docs/kafka-12) kèm thói quen backup trước khi sửa. [Thay đổi cấu hình động (Dynamic Configuration Changes)](#/docs/kafka-12) cho bốn entity-type: topics, brokers, users, clients. [Ghi đè cấu hình mặc định của topic (Overriding Topic Configuration Defaults)](#/docs/kafka-12) với Bảng 12-2 tra cứu; [Ghi đè cấu hình mặc định của client và user (Overriding Client and User Configuration Defaults)](#/docs/kafka-12) với Bảng 12-3; [Ghi đè cấu hình mặc định của broker (Overriding Broker Configuration Defaults)](#/docs/kafka-12) cho ba cấu hình đáng nhớ; rồi [Xem các cấu hình ghi đè (Describing Configuration Overrides)](#/docs/kafka-12) cùng khung "Chỉ hiển thị các giá trị ghi đè của topic", và [Gỡ bỏ các cấu hình ghi đè (Removing Configuration Overrides)](#/docs/kafka-12) — \`--alter\` cùng \`--delete-config\` đưa entity về mặc định cluster.
...
**Đọc.** [Produce và consume (Producing and Consuming)](#/docs/kafka-12) mở ngay bằng khung "Chuyển hướng output sang một ứng dụng khác". [Console Producer](#/docs/kafka-12) chạy bốn message rồi Control-D; [Sử dụng các tùy chọn cấu hình producer (Using producer configuration options)](#/docs/kafka-12) với khung "Các tùy chọn dòng lệnh dễ gây nhầm lẫn" và [Các tùy chọn của trình đọc dòng (Line-reader options)](#/docs/kafka-12). [Console Consumer](#/docs/kafka-12) cùng khung "Kiểm tra phiên bản công cụ", rồi [Sử dụng các tùy chọn cấu hình consumer (Using consumer configuration options)](#/docs/kafka-12) qua \`--consumer.config\` hoặc \`--consumer-property\`, và [Các tùy chọn của message formatter (Message formatter options)](#/docs/kafka-12) với Bảng 12-4 và [Consume các topic offset (Consuming the offsets topics)](#/docs/kafka-12). [Quản lý partition (Partition Management)](#/docs/kafka-12) rồi [Bầu chọn replica ưu tiên (Preferred Replica Election)](#/docs/kafka-12). [Thay đổi replica của một partition (Changing a Partition's Replicas)](#/docs/kafka-12) đọc chậm nhất tuần: ba bước generate, execute, verify cùng khung "Cải thiện việc sử dụng mạng khi gán lại replica"; rồi [Thay đổi replication factor (Changing the replication factor)](#/docs/kafka-12) và [Hủy các thao tác gán lại replica (Canceling replica reassignments)](#/docs/kafka-12). [Kết xuất các log segment (Dumping Log Segments)](#/docs/kafka-12) chạy thật cả hai dạng, có và không có \`--print-data-log\`. [Kiểm chứng replica (Replica Verification)](#/docs/kafka-12) khép mục — cả dòng lệnh lẫn output ví dụ đều bị bản PDF gốc cắt cụt: đừng gõ lại, đừng chờ thấy đúng chúng.

**Bẫy.** Chạy \`kafka-reassign-partitions.sh --execute\` trên cluster production mà không kèm \`--throttle\`. Sách nói rõ việc gán lại partition ảnh hưởng lớn tới hiệu năng cluster: đổi tính nhất quán memory page cache, tốn cả I/O mạng lẫn đĩa; \`--throttle\` nhận byte/giây và ghép được với \`--additional\` để hãm một lần gán lại đã lỡ bắt đầu. Bẫy thứ hai: bọc console consumer trong một ứng dụng để chuyển tiếp message. Khung "Chuyển hướng output sang một ứng dụng khác" gọi loại ứng dụng này là mong manh và nên tránh: rất khó tương tác với console consumer theo cách không làm mất message.
...
**Tự kiểm tra.** Vì sao metric dạng quantile bị loại khỏi nhóm SLI tốt, và bucket thay nó bằng cách nào? Và vì sao cảnh báo đặt thẳng trên ngưỡng SLO thì luôn nổ quá muộn?`,
      },
      {
        ...
        lesson: `**Mục tiêu.** Chẩn đoán ba nhóm sự cố cluster bằng vài metric, và đọc under-replicated partitions như triệu chứng cần truy tiếp, không phải chuông báo cháy.

**Đọc.** [Metric của Kafka Broker (Kafka Broker Metrics)](#/docs/kafka-13) mở bằng khung "AI SẼ GIÁM SÁT NGƯỜI GIÁM SÁT?". [Chẩn đoán sự cố cluster (Diagnosing Cluster Problems)](#/docs/kafka-13) cho ba nhóm — broker đơn lẻ, cluster quá tải, sự cố controller — cùng khung "BẦU CHỌN PREFERRED REPLICA": chạy trước, xem sự cố còn không. [Nghệ thuật của Under-Replicated Partitions](#/docs/kafka-13) đọc chậm nhất: Bảng 13-3, khung "CÁI BẪY CẢNH BÁO URP", cách phân biệt con số ổn định với con số dao động, và mẹo tìm broker chung trong output \`--under-replicated\` — sách in cờ rút gọn, cờ thật sự chạy được (học từ tuần 9) là \`--under-replicated-partitions\`. [Sự cố ở mức cluster (Cluster-level problems)](#/docs/kafka-13) với Bảng 13-4; [Sự cố ở mức host (Host-level problems)](#/docs/kafka-13) với khung "MỘT QUẢ TRỨNG HỎNG". [Broker Metrics](#/docs/kafka-13) gõ lại từng MBean, bám bốn mục: [Active controller count](#/docs/kafka-13), [Controller queue size](#/docs/kafka-13), [Request handler idle ratio](#/docs/kafka-13) với hai ngưỡng 20% và 10%, và [Offline partitions](#/docs/kafka-13) — loại sự cố sập dịch vụ, phải xử lý ngay. [All topics bytes in](#/docs/kafka-13) đọc kèm bảy thuộc tính của metric dạng rate; [All topics bytes out](#/docs/kafka-13) mang khung "CÓ TÍNH CẢ REPLICA FETCHER"; [All topics messages in](#/docs/kafka-13) mang khung "TẠI SAO KHÔNG CÓ MESSAGES OUT?". [Request metrics](#/docs/kafka-13) với Bảng 13-15 và khung "PERCENTILE LÀ GÌ?". [Giám sát JVM (JVM Monitoring)](#/docs/kafka-13), [Giám sát hệ điều hành (OS Monitoring)](#/docs/kafka-13) và [Logging](#/docs/kafka-13) đọc lướt hơn.

**Bẫy.** Đặt under-replicated partitions làm metric cảnh báo chính. Khung "CÁI BẪY CẢNH BÁO URP" rút lại chính lời khuyên của ấn bản trước: URP thường khác không vì lý do vô hại, nên bạn sẽ nhận cảnh báo giả rồi bỏ qua — hãy dựa vào cảnh báo theo SLO. Bẫy thứ hai: đẩy metric Kafka qua chính cluster đang giám sát. Khung "AI SẼ GIÁM SÁT NGƯỜI GIÁM SÁT?" nói thẳng: rất có khả năng bạn sẽ không bao giờ biết khi nào Kafka hỏng, bởi luồng dữ liệu cho hệ thống giám sát cũng hỏng theo.
...
**Tự kiểm tra.** Vì sao một phép đo dựng từ ngoài cluster lại là nguồn SLI tốt hơn mọi metric broker? Và nếu bạn vận hành cluster nhưng không vận hành client, việc thiếu giám sát đầu-cuối đẩy thêm gánh nặng gì lên bạn?`,
  ...
  },
  {
    ...
    resources: [
      { label: "Kafka 14 — Xử lý luồng", href: "#/docs/kafka-14" },
