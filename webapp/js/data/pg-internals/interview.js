...
---------------------------------
 0.2`,
  ...
  },
  {
    ...
    incident: {
      symptom:
        "Hệ thống giám sát báo cảnh age(relfrozenxid) của bảng audit_log — một bảng log lịch sử rất lớn, gần như chỉ insert-only — đang tiến sát ngưỡng cấu hình cho autovacuum cưỡng bức. Trong log server xuất hiện đều đặn các dòng autovacuum: VACUUM public.audit_log (to prevent wraparound), nhưng lần chạy nào cũng có vẻ không bao giờ dứt điểm được việc đẩy relfrozenxid lên.",
      scale:
...
(1 row)`,
  ...
  // ===== pg-plan (pg-iq17–pg-iq20) =====
  {
    ...
    minutes: 5,
    question:
      "Mô tả bốn giai đoạn một câu lệnh SQL trải qua trong giao thức truy vấn đơn giản của PostgreSQL: parse, transformation (rewrite), plan, execute. Prepared statement thay đổi điều này ra sao, và khi nào PostgreSQL chuyển từ custom plan sang generic plan?",
    mustCover: [
      ...
      "Giai đoạn transformation (rewrite): parse tree có thể bị biến đổi, ví dụ thay tên một view bằng truy vấn cơ sở của nó hoặc áp dụng row-level security — mọi thao tác vẫn diễn ra trên cây, không phải trên văn bản gốc",
      ...
      "Prepared statement tách riêng giai đoạn chuẩn bị (parse và transformation/rewrite, giữ lại parse tree trong bộ nhớ backend) khỏi giai đoạn gắn tham số và thực thi, tránh phải phân tích cú pháp lại cùng một truy vấn nhiều lần",
...
(1 row)`,
    ...
    question:
      "Trước khi tạo extended statistics, EXPLAIN ước lượng chỉ rows=15 cho Bitmap Heap Scan trên flights với hai điều kiện flight_no = 'PG0007' AND departure_airport = 'VKO', trong khi đếm trực tiếp cho ra 396 dòng thật sự khớp cả hai điều kiện. Chẩn đoán vì sao ước lượng lệch hơn 26 lần, giải thích CREATE STATISTICS (dependencies) sửa được gì, và vì sao con số ước lượng sau khi sửa (277) vẫn chưa khớp đúng 396.",
    mustCover: [
      "Planner mặc định giả định mọi điều kiện lọc kết hợp bằng AND là độc lập với nhau nên nhân riêng selectivity của flight_no = 'PG0007' với selectivity của departure_airport = 'VKO' — đây là vấn đề vị từ tương quan (problem of correlated predicates) kinh điển",
      "Node Bitmap Index Scan trên flights_flight_no_scheduled_departure_key đã ước lượng đúng gần khớp cho riêng điều kiện flight_no (rows=276), nhưng Bitmap Heap Scan nhân thêm selectivity riêng lẻ của departure_airport vào, kéo ước lượng tổng xuống chỉ còn rows=15 — thấp hơn con số thực 396 khoảng 26 lần",
      "Thực tế một số hiệu chuyến bay chỉ khởi hành từ đúng một sân bay, nên departure_airport gần như phụ thuộc hàm hoàn toàn vào flight_no chứ không độc lập — điều kiện thứ hai gần như dư thừa về mặt chọn lọc",
      "CREATE STATISTICS flights_dep (dependencies) ON flight_no, departure_airport FROM flights, theo sau bởi ANALYZE flights, ghi lại mức độ phụ thuộc hàm này vào pg_stats_ext — hệ số '2 => 5': 1.000000 nghĩa là cột thứ 5 (departure_airport) phụ thuộc hoàn toàn vào cột thứ 2 (flight_no)",
      "Sau khi có statistics mở rộng, ước lượng tăng lên rows=277 — gần bằng chính selectivity riêng của flight_no vì planner giờ coi điều kiện departure_airport gần như không bổ sung thêm thông tin chọn lọc; 277 gần con số thực 396 hơn hẳn so với 15 ban đầu, nhưng vẫn chỉ là một ước lượng chứ không khớp tuyệt đối, vì hệ số phụ thuộc hàm là một con số tổng quát chứ không phải phân bố giá trị cụ thể",
    ],
    model:
      "Trước khi có extended statistics, planner ước lượng cardinality của một truy vấn có hai điều kiện AND bằng cách nhân riêng selectivity của từng điều kiện, ngầm giả định flight_no = 'PG0007' và departure_airport = 'VKO' độc lập với nhau — đây chính là vấn đề vị từ tương quan (problem of correlated predicates) kinh điển. Trong EXPLAIN, node Bitmap Index Scan trên flights_flight_no_scheduled_departure_key ước lượng đúng gần khớp cho riêng điều kiện flight_no: rows=276. Nhưng khi Bitmap Heap Scan áp thêm Filter departure_airport = 'VKO' lên trên, nó nhân tiếp selectivity riêng lẻ của departure_airport vào, kéo ước lượng tổng xuống chỉ còn rows=15 (cost=10.49..816.84) — trong khi đếm trực tiếp bằng SELECT count(*) với cùng hai điều kiện cho ra 396 dòng, tức ước lượng thấp hơn thực tế khoảng 26 lần. Vấn đề nằm ở chỗ hai cột này không hề độc lập: mỗi số hiệu chuyến bay (flight_no) chỉ khởi hành từ đúng một sân bay, nên biết flight_no gần như đã biết luôn departure_airport — điều kiện thứ hai gần như dư thừa chứ không mang thêm thông tin chọn lọc độc lập. Cách sửa là dạy cho planner biết về mối phụ thuộc hàm này bằng CREATE STATISTICS flights_dep (dependencies) ON flight_no, departure_airport FROM flights, sau đó ANALYZE flights để thu thập statistic mới — một lần ANALYZE thường không kèm CREATE STATISTICS sẽ không sửa được gì, vì statistics ở mức cột không bao giờ ghi lại mối quan hệ giữa hai cột với nhau. Statistic thu thập được lưu trong pg_stats_ext dưới dạng hệ số mức độ phụ thuộc cho từng chiều: {'2 => 5': 1.000000, '5 => 2': 0.010200}, tức cột số 5 (departure_airport) phụ thuộc hàm hoàn toàn (hệ số 1.0) vào cột số 2 (flight_no), trong khi chiều ngược lại gần như không có (0.0102). Sau khi có statistic này, EXPLAIN cho ước lượng mới rows=277 (cost=10.57..819.51) — gần đúng bằng selectivity riêng của flight_no, vì planner giờ hiểu điều kiện departure_airport gần như không cắt giảm thêm gì so với chỉ lọc theo flight_no. 277 gần con số thực 396 hơn hẳn so với 15 ban đầu — sai số giảm từ khoảng 26 lần xuống chưa tới 1,5 lần — nhưng vẫn không khớp tuyệt đối, vì hệ số phụ thuộc hàm chỉ là một con số tổng quát (0 đến 1) mô tả MỨC ĐỘ phụ thuộc trung bình giữa hai cột, không phải một bảng tra cứu ghi lại chính xác phân bố từng cặp giá trị như statistics MCV đa biến — muốn khớp gần đúng hơn nữa với các cặp giá trị cụ thể, tôi sẽ cần CREATE STATISTICS ... (mcv) thay vì (dependencies).",
    redFlags: [
      ...
      "Nghĩ rằng sau khi tạo extended statistics, ước lượng phải khớp chính xác con số thực tế",
    ],
    probes: [
      "Vì sao một lần ANALYZE thường (không kèm CREATE STATISTICS) không sửa được ước lượng sai này?",
      "Vì sao ước lượng sau khi sửa (277) vẫn không khớp đúng con số thực (396)?",
...
(3 rows)`,
  ...
  },
  {
    ...
    tradeoffs: [
      {
        option: "B-tree",
        when: "Không dùng trực tiếp được cho ba trong bốn tình huống — nó chỉ hỗ trợ năm toán tử so sánh trên giá trị vô hướng có thứ tự, không hiểu toán tử 'chứa' trên jsonb, không tách được từ trong văn bản, và không tự nhiên biểu diễn được ràng buộc chồng lấn khoảng; với riêng việc lọc log theo thời gian, B-tree thực ra vẫn hiệu quả hơn BRIN (không lossy, không cần recheck) nhưng phải trả giá bằng kích thước: sách đo được B-tree trên cùng cột lớn hơn BRIN hơn một nghìn lần (210 MB so với 184 kB) — cái giá không kham nổi trên một bảng log cỡ terabyte, nên BRIN vẫn được ưu tiên; B-tree cũng hữu ích gián tiếp qua các extension btree_gist/btree_gin khi cần trộn một cột so sánh thường vào một index GiST hoặc GIN đa cột.",
      },
      {
        option: "GIN",
