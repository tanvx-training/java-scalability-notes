    ...
    items: [
      {
        ...
        lesson: `**Mục tiêu.** Kể đúng bốn giai đoạn của giao thức truy vấn đơn giản (parse, transformation (rewrite), plan, execute), đọc được vì sao planner loại bỏ một bảng khỏi cây plan trong ví dụ \`pg_tables\`, và giải thích ý nghĩa của startup cost/total cost trong output \`EXPLAIN\`.
...
**Bẫy.** Nghĩ tăng \`siglen\` luôn "miễn phí" để giảm false positive. Sách chứng minh bằng số liệu thật (127 MB → 139 MB khi tăng \`siglen\` từ mặc định 124 lên 248, đồng thời \`Rows Removed by Index Recheck\` giảm từ 7859 xuống 2060) rằng độ chính xác đánh đổi trực tiếp bằng kích thước index. Bẫy thứ hai: nghĩ vì GiST hỗ trợ toán tử >, < và = qua \`btree_gist\` nên nó là lựa chọn tương đương B-tree cho dữ liệu có thứ tự. Sách cảnh báo rõ B-tree vẫn hiệu quả hơn nhiều cho việc này, và \`btree_gist\` chỉ nên dùng khi cần kết hợp một cột có thứ tự vào cùng index nhiều cột với một cột GiST-only (ví dụ exclusion constraint).
...
**Tự kiểm tra.** Với ba bài toán — tìm kiếm jsonb bằng \`@>\`, một bảng log append-only một tỷ dòng cần lọc theo thời gian, và một ràng buộc "lịch phòng không được chồng lấn" — bảng một trang của bạn chỉ ra loại index nào cho mỗi bài toán, và vì sao BRIN được ưu tiên hơn B-tree cho bài toán log dù B-tree vẫn hoạt động chính xác trên cột thời gian đó? Nếu phải đổi loại index đã chọn vì bảng ghi nhiều hơn dự kiến, chương nào trong số 24–29 cho biết cái giá cụ thể phải trả khi ghi tăng lên?`,
      },
    ],
  },
];
