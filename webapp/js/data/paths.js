...
export const PATHS = {
  ...
  java: {
    label: "Java Backend",
    icon: "☕",
    desc: "Một nghề, mười chặng: Spring cơ bản → Java hiện đại → dùng ngôn ngữ cho đúng (API, generics, equals/hashCode, exception) → xuống dưới nắp JVM (bytecode, JMM, build, container) → nền concurrency cổ điển (thread safety, lock, AQS, JMM) → đo và tối ưu trên cloud (GC, JIT, observability, profiling) → persistence (ánh xạ, persistence context, transaction, fetch) → khả năng mở rộng trên Tomcat → concurrency sau Loom → bảo mật. Lập trình hệ thống là nền tuỳ chọn cho ai muốn hiểu tới tầng kernel.",
